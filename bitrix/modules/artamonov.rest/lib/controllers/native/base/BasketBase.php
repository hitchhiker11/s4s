    ...
    protected function formatBasket(Sale\Basket $basket, string $format = 'full'): array
    {
        ...
        while ($row = $result->fetch()) {
            ...
            if ($format === 'full') {
                ...
                $productInfo = $this->getProductInfo($row['PRODUCT_ID']);
                $itemData['detail_page_url'] = $productInfo['detail_page_url'];
                $itemData['product_xml_id'] = $productInfo['xml_id'];
                $itemData['picture'] = $productInfo['picture'];
            }
    ...
    protected function getProductInfo(int $productId): array
    {
        ...
        try {
            ...
            $element = \CIBlockElement::GetList(
                ...
                false,
                false,
                ['ID', 'IBLOCK_ID', 'DETAIL_PAGE_URL', 'XML_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'CODE', 'NAME']
            )->Fetch();
            ...
            if ($element) {
                ...
                if (!empty($element['DETAIL_PAGE_URL'])) {
                    ...
                    // Обрабатываем шаблоны в URL
                    if (strpos($detailUrl, '#') !== false) {
                        // Заменяем #SITE_DIR# на корень сайта
                        $detailUrl = str_replace('#SITE_DIR#', '', $detailUrl);
                        
                        // Заменяем #ELEMENT_CODE# на реальный код элемента
                        if (!empty($element['CODE'])) {
                            $detailUrl = str_replace('#ELEMENT_CODE#', $element['CODE'], $detailUrl);
                        } else {
                            // Если кода нет, заменяем на ID
                            $detailUrl = str_replace('#ELEMENT_CODE#', $productId, $detailUrl);
                        }
                        
                        // Заменяем #ELEMENT_ID# если есть
                        $detailUrl = str_replace('#ELEMENT_ID#', $productId, $detailUrl);
                        
                        // Убираем дублирующие слэши
                        $detailUrl = preg_replace('/\/+/', '/', $detailUrl);
                        
                        // Если URL не начинается с /, добавляем
                        if (!str_starts_with($detailUrl, '/')) {
                            $detailUrl = '/' . $detailUrl;
                        }
                    }
                    
                    $productInfo['detail_page_url'] = $detailUrl;
                } else {
                    // Если прямой ссылки нет, формируем её из кода товара
                    if (!empty($element['CODE'])) {
                        $productInfo['detail_page_url'] = '/catalog/product/' . $element['CODE'] . '/';
                    } else {
                        $productInfo['detail_page_url'] = '/catalog/product/' . $productId . '/';
                    }
                }

                $productInfo['xml_id'] = $element['XML_ID'];
                ...
                if ($pictureId) {
                    ...
                    if ($picture) {
                        $productInfo['picture'] = [
                            ...
                            'alt' => $picture['ALT'] ?: $element['NAME'] ?? '',
                            'title' => $picture['TITLE'] ?: $element['NAME'] ?? ''
                        ];
                    }
                }
            }
