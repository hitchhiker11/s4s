...
<?\Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID("menu-default");?>
<script>
...
$(document).ready(function($) {
	...
	<?foreach ($arResult as $key => $arItem):?>
		<?if($arItem["SELECTED"]):?>
			$(".main-menu .section-menu-id-"+<?=$arItem["ID"]?>).addClass('selected');
		<?endif;?>
		...
		<?if(!empty($arItem["SUB"]) && is_array($arItem["SUB"])):?>
	    	...
	    	<?foreach($arItem["SUB"] as $key1=>$arSub):?>
	        	...
	        	<?if($arSub["SELECTED"]):?>
	        		$(".main-menu .section-menu-id-"+<?=$arSub["ID"]?>).addClass('selected');
				<?endif;?>
	        	...
	        	<?if(!empty($arSub["SUB"]) && is_array($arSub["SUB"])):?>
	                ...
	                <?foreach($arSub["SUB"] as $key2=>$arSub2):?>
	                	...
	                	<?if($arSub2["SELECTED"]):?>
							$(".main-menu .section-menu-id-"+<?=$arSub2["ID"]?>).addClass('selected');
						<?endif;?>
...
<?\Bitrix\Main\Page\Frame::getInstance()->finishDynamicWithID("menu-default");?>
