<style>
  ...
  .k1-search-panel__categories {
    & .k1-search-panel__categories-text {
    ...
    <form action="/search/catalog/" class="search-form k1-catalog-search-form k1-catalog-search-form-js">
      <div class="k1-search-panel">
        ...
        <div class="k1-search-panel__button">
          <button
            class="button-def search-btn-style main-color <?= $KRAKEN_TEMPLATE_ARRAY["BTN_VIEW"]['VALUE'] ?>"
            type="submit">🡨 Найти
          </button>
        </div>
      </div>
...
<div class="container">
  <?php
    ...
    if (!str_contains($APPLICATION->GetCurPage(), "search/catalog")) {
      if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/bitrix/templates/concept_kraken_s1/include/orderTableDescription.php')) {
        require_once($_SERVER['DOCUMENT_ROOT'] . '/bitrix/templates/concept_kraken_s1/include/orderTableDescription.php');
      }
    }
...
<script>
  window.addEventListener("DOMContentLoaded", () => {
    const categoriesButton = document.querySelector(".k1-search-panel__categories-text")
    ...
    function showCheckBoxes() {
      ...
      categoriesButton.textContent = showText
      setTimeout(function() {
    ...
    if (categoriesButton && checkBoxesWrapper) {
