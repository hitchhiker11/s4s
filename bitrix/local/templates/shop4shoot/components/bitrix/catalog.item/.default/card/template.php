 ...
 * @var array $morePhoto
 * @var bool $showSlider
 * @var bool $itemHasDetailUrl * @var string $imgTitle
...
<div class="product-item k1-test">
	<? if ($itemHasDetailUrl): ?>
...
<script>
  var <?=$arResult['AREA_ID'].'mustache'?> = {
    ...
    DISCOUNT_PRICE_FORMATED: '<?=$option['DISCOUNT_PRICE']?> руб.',
    DISCOUNT_PRICE_PERCENT: <?=$option['DISCOUNT']?>,
    DISCOUNT_PRICE_PERCENT_FORMATED: '<?=$option['DISCOUNT']?>%',
    FULL_PRICE: <?=$option['PRICE']?>,
    FULL_PRICE_FORMATED: '<?=$option['PRICE']?> руб.',
    HASH: '',
    ...
    NOT_AVAILABLE: false,
    PRICE: <?=$price['PRICE']?>,
    PRICE_FORMATED: '<?=$price['PRICE']?> руб.',
    PRODUCT_ID: <?=$item['ID']?>,
    ...
    SKU_BLOCK_LIST: {},
    SORT: "100",
    SUM_DISCOUNT_PRICE: <?=$price['DISCOUNT']?>,
    SUM_DISCOUNT_PRICE_FORMATED: '<?=$price['DISCOUNT']?> руб.',
    SUM_FULL_PRICE: <?=$price['PRICE']?>,
    SUM_FULL_PRICE_FORMATED: '<?=$price['PRICE_FULL']?> руб.',
    SUM_PRICE: <?=$price['PRICE']?>,
    SUM_PRICE_FORMATED: '<?=$price['PRICE']?> руб.',
    WARNINGS: {},
    //ID: //товара корзины !дописываем уже в js скрипте, как получим его после добавления товара в корзину
  };
</script>