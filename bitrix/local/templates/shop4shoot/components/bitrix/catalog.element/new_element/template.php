<?php
  ...
  if ($basketItems) {
    foreach ($basketItems as $item) {
      if ($item->getField("PRODUCT_ID") == $arResult["ID"]) {
        $inBasket = true;
        $inBasketQuantity = (int)$item->getField("QUANTITY");
        break;
      }
    }
  }
  ...
  foreach ($arResult["OFFERS"] as $offer) {
    $id = $offer["ID"];
    $photoIDs = $offer["PROPERTIES"]["MORE_PHOTO"]["VALUE"];
    $photoArray = [];
    if (!empty($photoIDs)) {
      foreach ($photoIDs as $photo) {
        $photoArray[] = CFile::GetPath($photo);
      }
    }
    foreach ($arResult['JS_OFFERS'] as $key => $jsOffer) {
      ...
      if ($id === $jsID) {
        if (!empty($photoArray)) {
          $arResult['JS_OFFERS'][$key]["MORE_PHOTO"] = $photoArray;
        }
        $arResult['JS_OFFERS'][$key]["ARTICLE"] = $offer["PROPERTIES"]["ARTICLE"]["VALUE"];
        continue;
      }
    }
  }
  ...
  if (!empty($actualItem["PROPERTIES"]["MORE_PHOTO"]["VALUE"])) {
    foreach ($actualItem["PROPERTIES"]["MORE_PHOTO"]["VALUE"] as $photoID) {
      $galleryPhotos[] = CFile::GetPAth($photoID);
    }
  }
  if ($arResult["DETAIL_PICTURE"]["SRC"]) {
    $galleryPhotos[] = $arResult["DETAIL_PICTURE"]["SRC"];
  }
  ...
  if (empty($galleryPhotos)) {
    $galleryPhotos[] = CATALOG_PICTURE_STUB;
    $hasGallery = false;
  }
//  var_dump($galleryPhotos);
  $this->SetViewTarget('catalog-left-menu'); ?>
<?php if (!empty($arResult["SIDE_MENU"])): ?>
  <ul class='nav'>
    
    <?php foreach ($arResult["SIDE_MENU"] as $key => $value): ?>
      
      <?php if (strlen($value["NAME"]) > 0): ?>
        <li>
          <a href="#<?= $key ?>"
             class='scroll <?php if ($key == "main"): ?>review<?php endif; ?>'><?= $value["NAME"] ?></a>
        </li>
      <?php endif; ?>
    ...
    <li class="back">
      <a
        href="<?= $arResult["SECTION"]["SECTION_PAGE_URL"] ?>">К списку товаров</a>
    </li>
...
<?php $this->EndViewTarget(); ?>
  ...
  <div class="bx-catalog-element bx-<?= $arParams['TEMPLATE_THEME'] ?>" id="<?= $itemIds['ID'] ?>"
    ...
    <div class="container-fluid">
      ...
      ?>
      <h1 class="bx-title"><?= $name ?></h1>
      <div class="good-data scroll-next-parent">
        <div class="row">
          <div class="col-md-6 col-sm-5 col-xs-12 parent-show-pic">
            
            <div class="pics-block">
              
              <div id="bx_117848907_2508_detail_gallery_block">
                <table class="big-pic">
                  <tbody>
                  <tr>
                    <td class="gallery-list">
                      <?php foreach ($galleryPhotos as $key => $photo) : ?>
                        <div class="item-show-pic <?php if ($key == 0): ?>active<?php endif; ?>"
                             data-show-pic="img-<?= $key ?>">
                          <a href="<?= $photo ?>"
                             data-gallery="gal-item"
                             class="cursor-loop">
                            <img class="img-responsive center-block animate_to_box" data-cart-id-img="2618"
                                 title="<?= $arResult["NAME"] ?>"
                                 alt="<?= $arResult["NAME"] ?>"
                                 src="<?= $photo ?>">
                          </a>
                ...
                <?php if ($hasGallery) : ?>
                  <div class="tabs-pic">
                    ...
                    <div class="row gallery-control k1">
                      <?php foreach ($galleryPhotos as $key => $photo) : ?>
                        <div class="col-xs-3 pic-item">
                          <div class="mini-pic click-show-pic <?= $key === 0 ? "active" : "" ?>"
                               data-show-pic="img-<?= $key ?>"
                               title="Кобура Super Ghost Ultimate Evo">
                            <table>
                              <tbody>
                              <tr>
                                <td>
                  ...
                  <div class="info-stoke-wrap row clearfix">
                    <div class="col-sm-8 col-xs-6">
                      <div class="article italic">
                        <?php if (!$haveOffers) {
                          echo "Артикул: " . $arResult["PROPERTIES"]["ARTICLE"]["VALUE"];
                        } else {
                          echo "Артикул: " ?><span class="offer-article"></span><?php
                        } ?>
                      </div>
                      <span itemprop="additionalProperty" itemscope="" itemtype="http://schema.org/PropertyValue">
                        <meta itemprop="value" content="18-102389"><meta itemprop="name" content="Артикул:">
                      </span>
                    </div>
                    <div class="stoke-wrap col-sm-4 col-xs-6">
                      <?php
                        $isAvailable = $arResult["PRODUCT"]["AVAILABLE"] === "Y";
                      ?>
                      <div class="stoke <?= $isAvailable ? "in" : "out" ?>">
                        ...
                        case 'price':
                          ...
                          <div class="product-item-detail-info-container">
                            ...
                            <div class="price-table">
                              <div class="price price-cell big">
                                <div class="product-item-detail-price-current" id="<?= $itemIds['PRICE_ID'] ?>">
                                  <span class="bold"><?= $price['PRINT_RATIO_PRICE'] ?></span>
                                </div>
                              </div>
                            </div>
                            <?php
                        ...
                        case 'quantity':
                          if ($arParams['USE_PRODUCT_QUANTITY']) {
                            ...
                            <div class="product-item-detail-info-container"
                              ...
                              <div
                                class="product-item-detail-info-container-title"><?= Loc::getMessage('CATALOG_QUANTITY') ?></div>
                              ...
                              <div id="<?= $itemIds['BASKET_ACTIONS_ID'] ?>"
                                ...
                                <?php
                                  if ($showAddBtn) {
                                    ...
                                    <div class="product-item-detail-info-container">
                                      <a
                                        class="button-def main-color elips <?= $showButtonClassName ?>
                                        product-item-detail-buy-button"
                                        id="<?= $itemIds['ADD_BASKET_LINK'] ?>"
                                        href="javascript:void(0);">
                                        <span>Добавить в корзину</span>
                                        <!--                                      <span>-->
                                        <?php //= $arParams['MESS_BTN_ADD_TO_BASKET'] ?><!--</span>-->
                                      </a>
                                    </div>
                                    <?php
                                  }
                                  ...
                                  if ($showBuyBtn) {
                                    ?>
                                    <div class="wrap-btn">
                                      <div class="product-item-detail-info-container k1-buttons-block">
                                        <a
                                          class="r-button r-button-product-element <?= $inBasket ? "r-button-disabled"
                                            : ""
                                          ?>"
                                          id="<?= $itemIds['BUY_LINK'] ?>"
                                          href="javascript:void(0);">
                                        ...
                                        <?php if ($USER->IsAdmin()) : ?>
                                          <div class="k-element-count <?= $inBasket ? "added" : "" ?>"
                                               data-k-id="<?= $arResult["ID"]?>"
                                               data-k-max="<?= $arResult["CATALOG_QUANTITY"]?>"
                                          >
                                          <table>
                                            <tbody>
                                            <tr>
                                              ...
                                              <td class="right plus k-count-plus <?= $inBasketQuantity >= $arResult["CATALOG_QUANTITY"]
                                                ? "disabled" : "" ?>">
                                                <div>
                                                </div>
                                              </td>
...
<?php
  $headerPicture =
    $arResult["PROPERTIES"]["HEADER_PICTURE"]["VALUE"]
      ? CFile::GetPath($arResult["PROPERTIES"]["HEADER_PICTURE"]["VALUE"])
